...
In the `template.json` file, the properties are configured to extract values as follow:
```jsonc
"Arguments": {
  "DeviceIdExpression": "utf8[0:2]",   // treat first two bytes as a string
  "LatitudeExpression": "i16le[2:4]",  // lat is second pair of bytes as 16 bit integer in little-endian format
  "LongitudeExpression": "i16le[4:6]", // next two are lon
  "PositionPropertyExpressions": {
    "foo": "u16le[6:8]", // user-defined property 1, unsigned 16 bit LE integer
    "bar": "utf8[8:10]"  // user-defined property 2, text
  }
}
```

BREX provides a flexible method for binary data extraction, supporting simple ranges, indirect indexes, TLV search, conditionals and error handling (defaults).

Latitude and longitude are provided to the AWS API as `double` values, with values converted from the BREX extracted values and scaled accordingly.  See [the docs](https://proxylity.com/docs/destinations/location.html) for the full details.