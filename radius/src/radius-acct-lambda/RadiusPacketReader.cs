...
public static class RadiusPacketReader
{
    ...
    internal static IEnumerable<(byte type, Memory<byte> value)> Attributes(Memory<byte> packet, bool include_vsas = false)
    {
        if (packet.Length < 20)
            yield break;
    ...
    internal static IEnumerable<(uint vendorid, Memory<byte> value)> VendorSpecificAttributes(Memory<byte> packet)
    {
        if (packet.Length < 20)
            yield break;
    ...
    public static (byte code, short length, byte id, Memory<byte> authenticator, IEnumerable<(byte type, Memory<byte> value)> attributes, IEnumerable<(uint vendorid, Memory<byte> value)> vsas) Parse(Memory<byte> packet)
    {
        if (packet.Length < 20)
